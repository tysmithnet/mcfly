...
namespace McFly
{
    ...
    public class RegisterFacade : IRegisterFacade
    {
        ...
        public Regex GetRegisterRegex(Register register)
        {
            ...
            if (register == Register.Xmm0) return new Regex(@"(^|[ ,])xmm0=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            ...
            if (register == Register.Xmm1) return new Regex(@"(^|[ ,])xmm1=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Xmm10) return new Regex(@"(^|[ ,])xmm10=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            ...
            if (register == Register.Xmm11) return new Regex(@"(^|[ ,])xmm11=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            ...
            if (register == Register.Xmm12) return new Regex(@"(^|[ ,])xmm12=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            ...
            if (register == Register.Xmm13) return new Regex(@"(^|[ ,])xmm13=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            ...
            if (register == Register.Xmm14) return new Regex(@"(^|[ ,])xmm14=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            ...
            if (register == Register.Xmm15) return new Regex(@"(^|[ ,])xmm15=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            ...
            if (register == Register.Xmm2) return new Regex(@"(^|[ ,])xmm2=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            ...
            if (register == Register.Xmm3) return new Regex(@"(^|[ ,])xmm3=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            ...
            if (register == Register.Xmm4) return new Regex(@"(^|[ ,])xmm4=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            ...
            if (register == Register.Xmm5) return new Regex(@"(^|[ ,])xmm5=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            ...
            if (register == Register.Xmm6) return new Regex(@"(^|[ ,])xmm6=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            ...
            if (register == Register.Xmm7) return new Regex(@"(^|[ ,])xmm7=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            ...
            if (register == Register.Xmm8) return new Regex(@"(^|[ ,])xmm8=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            ...
            if (register == Register.Xmm9) return new Regex(@"(^|[ ,])xmm9=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            ...
            if (register == Register.Ymm0) return new Regex(@"(^|[ ,])ymm0=(?<val>(\s+[a-f0-9]+){8})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm0h) return new Regex(@"(^|[ ,])ymm0h=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm0l) return new Regex(@"(^|[ ,])ymm0l=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm1) return new Regex(@"(^|[ ,])ymm1=(?<val>(\s+[a-f0-9]+){8})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm10) return new Regex(@"(^|[ ,])ymm10=(?<val>(\s+[a-f0-9]+){8})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm10h) return new Regex(@"(^|[ ,])ymm10h=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm10l) return new Regex(@"(^|[ ,])ymm10l=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm11) return new Regex(@"(^|[ ,])ymm11=(?<val>(\s+[a-f0-9]+){8})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm11h) return new Regex(@"(^|[ ,])ymm11h=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm11l) return new Regex(@"(^|[ ,])ymm11l=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm12) return new Regex(@"(^|[ ,])ymm12=(?<val>(\s+[a-f0-9]+){8})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm12h) return new Regex(@"(^|[ ,])ymm12h=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm12l) return new Regex(@"(^|[ ,])ymm12l=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm13) return new Regex(@"(^|[ ,])ymm13=(?<val>(\s+[a-f0-9]+){8})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm13h) return new Regex(@"(^|[ ,])ymm13h=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm13l) return new Regex(@"(^|[ ,])ymm13l=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm14) return new Regex(@"(^|[ ,])ymm14=(?<val>(\s+[a-f0-9]+){8})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm14h) return new Regex(@"(^|[ ,])ymm14h=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm14l) return new Regex(@"(^|[ ,])ymm14l=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm15) return new Regex(@"(^|[ ,])ymm15=(?<val>(\s+[a-f0-9]+){8})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm15h) return new Regex(@"(^|[ ,])ymm15h=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm15l) return new Regex(@"(^|[ ,])ymm15l=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm1h) return new Regex(@"(^|[ ,])ymm1h=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm1l) return new Regex(@"(^|[ ,])ymm1l=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm2) return new Regex(@"(^|[ ,])ymm2=(?<val>(\s+[a-f0-9]+){8})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm2h) return new Regex(@"(^|[ ,])ymm2h=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm2l) return new Regex(@"(^|[ ,])ymm2l=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm3) return new Regex(@"(^|[ ,])ymm3=(?<val>(\s+[a-f0-9]+){8})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm3h) return new Regex(@"(^|[ ,])ymm3h=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm3l) return new Regex(@"(^|[ ,])ymm3l=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm4) return new Regex(@"(^|[ ,])ymm4=(?<val>(\s+[a-f0-9]+){8})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm4h) return new Regex(@"(^|[ ,])ymm4h=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm4l) return new Regex(@"(^|[ ,])ymm4l=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm5) return new Regex(@"(^|[ ,])ymm5=(?<val>(\s+[a-f0-9]+){8})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm5h) return new Regex(@"(^|[ ,])ymm5h=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm5l) return new Regex(@"(^|[ ,])ymm5l=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm6) return new Regex(@"(^|[ ,])ymm6=(?<val>(\s+[a-f0-9]+){8})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm6h) return new Regex(@"(^|[ ,])ymm6h=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm6l) return new Regex(@"(^|[ ,])ymm6l=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm7) return new Regex(@"(^|[ ,])ymm7=(?<val>(\s+[a-f0-9]+){8})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm7h) return new Regex(@"(^|[ ,])ymm7h=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm7l) return new Regex(@"(^|[ ,])ymm7l=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm8) return new Regex(@"(^|[ ,])ymm8=(?<val>(\s+[a-f0-9]+){8})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm8h) return new Regex(@"(^|[ ,])ymm8h=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm8l) return new Regex(@"(^|[ ,])ymm8l=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm9) return new Regex(@"(^|[ ,])ymm9=(?<val>(\s+[a-f0-9]+){8})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm9h) return new Regex(@"(^|[ ,])ymm9h=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
            if (register == Register.Ymm9l) return new Regex(@"(^|[ ,])ymm9l=(?<val>(\s+[a-f0-9]+){4})", RegexOptions.IgnoreCase);
