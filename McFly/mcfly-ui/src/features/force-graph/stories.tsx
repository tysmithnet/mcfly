...
interface State {nodes: ForceGraphNode[], links: ForceGraphLink[]}

export function generateRandomGraph(numNodes:number, numLinks:number):{nodes:ForceGraphNode[], links:ForceGraphLink[]} {
...
{
    public render(){
        return <ForceGraph id="a" width={window.innerWidth/2} height={window.innerHeight / 2} nodes={[]} links={[]} />
    }
}
...
{
    public render(){
        return <ForceGraph id="a" width={window.innerWidth/2} height={window.innerHeight / 2} nodes={[{id:"a"}]} links={[]} />
    }
}
...
export class SmallGraph extends Component<{}, {}>
{
    public render(){
        ...
        return <ForceGraph id="a" width={window.innerWidth/2} height={window.innerHeight / 2} nodes={data.nodes} links={data.links} />
    }
}
...
function addNodesReducer(state: State = {nodes:[], links:[]}, action: Action): State {
    switch(action.type){
        case "ADD_NODE":
            const node = {id:v4()};
...
class AddNodesImpl extends Component<{}, {}>
{
    ...
    public render(){
        ...
        return (
            <div>
                <ForceGraph id="a" width={window.innerWidth/2} height={window.innerHeight / 2} nodes={state.nodes} links={state.links} />
